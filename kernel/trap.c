...
void do_coprocessor_segment_overrun(unsigned long rsp,
                                    unsigned long error_code) {
  unsigned long *p = NULL;
  p = (unsigned long *)(rsp + 0x98);
  color_printk(RED, BLACK,
               "do_coprocessor_segment_overrun(9), "
...
void do_virtualization_exception(unsigned long rsp, unsigned long error_code) {
  ...
  p = (unsigned long *)(rsp + 0x98);
  color_printk(RED, BLACK,
               "do_virtualization_exception(20), "
               "ERROR_CODE:%#018lx,RSP:%#018lx,RIP:%#018lx\n",
               error_code, rsp, *p);
  while (1)
    ;
